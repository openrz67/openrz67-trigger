...
Net classes: `gnd` (GND, 0.13 track), `3v` (VCC, VDDA, 0.20), `5v` (VBUS, BAT+, BAT_CELL, VSYS, SW_SYS,
...
- **Review round 2026-09-25** (`notes/review-2026-09-25.md`), applied in the same source:
  - **Q3 reverse-battery block.** AO3401A P-MOSFET (SOT-23, LCSC C15127, basic) between BAT1
    pin 1 and everything else: drain on the cell (new net `BAT_CELL`), source on `BAT+`
  ...
  - **Paste windows on the exposed pads.** U1's 3.7 mm EP has no paste on the pad itself and
    ...
    library footprints and the board copies alike, so `lib_footprint_mismatch` stays quiet.
  - **RF line 0.8 mm** from the U.FL pad through C1/L1/C2 (was 0.5): at the 0.127 mm pour gap
    on 1.6 mm FR-4 that is roughly 52 Ω instead of ~60 Ω (hand calculation, coplanar with
    ground, no solder mask). The 0.3 mm tail into U1 pin 1 is unchanged; it cannot be wider
    between the 0.5 mm pitch pads.
  - **S2_DRV via 0.45/0.25** (was 0.4/0.2, which JLCPCB surcharges: 0.2 or 0.25 mm hole with a
    diameter under 0.45 costs more). It sits in a 0.70 mm slot between U1 pad 13, R23 pad 2 and
    the VCC diagonal, at (144.605, 104.295), 0.13–0.14 mm from each.
  - Bottom silk `v3`, a `+` at BAT1 pin 1 on the top silk, and the schematic note now says
    S2 = GPIO6 / VBAT_SENSE = GPIO3.
  - **Accepted, not changed** (why, in the notes file): no ESD array on USB-C (no room in the
    corner without moving the USB resistors; the bottom side sits 0.5 mm over the cell's foam;
    the C3's own pins are ±2 kV HBM behind 22 Ω, and D± only carry the bench build); BQ25185
    IN/SYS/BAT caps 5–10 mm from the pins (linear charger, no switching loop; the 10 µF are still
    the transient sink); the CHIP_EN RC 11 mm from pin 7 (the strip below the pad row is full);
    the π-match at the U.FL end and untuned (7 mm ≈ λ/10, matching would need a VNA); no test
    points (the bottom faces the cell, the top is full); 0.10 mm via annular ring (0.45/0.25 is
    what the two fabricated revisions used and is JLCPCB's no-surcharge floor).
- Still open from the 2026-09-08 review and 2026-09-10 (see above): one-via ground islands,
  0.16 mm power necks, U3 mask dam, the 1 µF on VDD_SPI.

## Ordering

Fab notes for the order form: 2 layers, 1.6 mm FR-4, 1 oz copper, no impedance control (the
RF line is a hand-calculated coplanar line, see rev 3), smallest via 0.45/0.25 mm, so pick the
0.25 mm minimum-hole option if the form asks. Q3 (SOT-23) is new in rev 3: check its pin-1
dot in the JLCPCB preview like U1/U5/U6 (`ROT_FIX` below).
